...
* **Self-contained deployment** — deploys via `dotnet publish` (self-contained or framework-dependent), producing a real native executable either way, with the executable bit restored after transfer (lost by default over tar/scp from Windows).
...
* Remote debugging*
  * _Attach/launch is still evolving. Please use VS' Attach to Process if you have issues._
...
3. VS > Extensions > Linux Debugger > **Build, Deploy, Debug**
